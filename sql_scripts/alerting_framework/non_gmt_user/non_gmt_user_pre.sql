/*select distinct coalesce(tag.tag_value,u.owner) as owner
...
select distinct coalesce(tag.owner,u.owner) as owner
from snowflake.account_usage.users u
left join CONTROL_DB.UTILITY.ROLE_TO_UC_TAGS_MAPPING_TABLE tag on lower(tag.role_name) = lower(u.owner)
where disabled = false
and (
...
-- and  coalesce(tag.owner,u.owner) <> 'snflak_anlytc_prj_gbl_gbl@dl.mgd.novartis.com'                
order by 1;